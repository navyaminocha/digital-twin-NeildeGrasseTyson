...
with col1:
    # Pre-fill text input with voice result if available
    typed_question = st.text_input(
        "Ask Neil (type here or use the mic below)",
...
if question:
    ...
    with col_a:
        speak = st.button("▶️ Play")
    if speak:
        audio_bytes = text_to_speech_audio(answer)
        if audio_bytes:
...
for item in reversed(st.session_state.history[-3:]):
    ...
    st.markdown(f"**Neil:** {item['assistant']}")
    st.markdown("---")